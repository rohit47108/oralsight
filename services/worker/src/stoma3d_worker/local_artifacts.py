...
SURFACE_ALGORITHM_VERSION = "stoma3d-observation-surface/2.1.0"
VIDEO_RENDERER_VERSION = "stoma3d-summary-video/3.0.0"
...
def build_observation_surface_from_evidence(
    ...
    *,
    ...
    manifest: dict[str, Any] = {
        "schemaVersion": "stoma3d.observation-surface.v2",
...
def _render_slide(
    *, title: str, body: str, caption: str, index: int, total: int
    ...
    draw.text((72, 54), "Stoma3D", font=label_font, fill="#0B716C")
...
GUIDANCE_COPY: dict[str, tuple[str, str, str]] = {
    ...
    "continue_user_selected_tracking": (
        ...
        "The reminder shown in Stoma3D was chosen by the user. Do not wait for "
...
def _render_observation_slide(
    ...
    *,
    ...
    draw = ImageDraw.Draw(image)
    draw.rectangle((0, 0, 1280, 14), fill="#0B716C")
    draw.text((62, 43), "Stoma3D", font=_font(19, bold=True), fill="#0B716C")
    draw.text(
        (62, 82),
...
def build_summary_video(
    ...
    *,
    ...
    with TemporaryDirectory(prefix="stoma3d-video-") as temporary:
        temp = Path(temporary)
        data = _encode_slides(
            slides,
    ...
    manifest: dict[str, Any] = {
        "schemaVersion": "stoma3d.summary-video.v3",
    ...
    return LocalArtifact(
        data=data,
        filename=f"stoma3d-summary-{payload.scan_session_id}.mp4",
