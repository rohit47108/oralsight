...
class WorkerResultNotification(ApiModel):
    schema_version: Literal["oralsight.job.v1"]
    job_id: str
...
class DeletionExecuteRequest(ApiModel):
    ...
    scope: Literal["all_oralsight_data"]
...
class ExportRenderResponse(ApiModel):
    ...
    status: Literal["complete"]
    artifact_id: str
    media_type: Literal["application/vnd.oralsight.export"]
    sha256: Sha256
...
class DataExportArtifactResponse(ApiModel):
    ...
    job_id: str
    media_type: Literal["application/vnd.oralsight.export"] = (
        "application/vnd.oralsight.export"
    )
    sha256: Sha256
    byte_size: int
    included_files: bool
