...
def legacy_deletion_receipt_fingerprint(settings: Settings, subject: str) -> str:
    ...
    return _digest(
        key.encode("utf-8"),
        f"oralsight:deletion-status-subject:v1:{subject}",
    )
...
def current_deleted_subject_fingerprint(settings: Settings, subject: str) -> str:
    ...
    return _digest(
        ...
        f"oralsight:deleted-subject-tombstone:v1:{identity}",
    )
    ...
    for version, key in configured_tombstone_keys(settings).items():
        if version == LEGACY_SHARE_KEY_VERSION:
            candidates[version] = _digest(
                key, f"oralsight:deletion-status-subject:v1:{subject}"
            )
        else:
            candidates[version] = _digest(
                key,
                f"oralsight:deleted-subject-tombstone:v1:{issuer_subject}",
            )
    return candidates
