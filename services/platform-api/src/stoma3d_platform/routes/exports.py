...
async def render_export(
    ...
    if (
        ...
        raise ServiceError(413, "export_too_large", "The portable export is too large.")
    artifact_id = new_id()
    object_key = f"users/{user.id}/exports/{artifact_id}.stoma3d-export"
    try:
        await request.app.state.object_storage.put_bytes(
            object_key,
            encrypted.ciphertext,
            media_type="application/vnd.stoma3d.export",
...
async def get_export_content(
    ...
    return Response(
        ...
        headers={
            ...
            "Content-Disposition": f'attachment; filename="stoma3d-export-{value.export_request_id}.bin"',
        },
    )
