...
Next.js App Router site for the public Stoma3D pages plus authenticated patient,
...
pnpm --filter @stoma3d/web verify
```

Set `NEXT_PUBLIC_SITE_URL` to the public HTTPS origin before a production build.
If omitted, metadata, robots, and sitemap use `https://stoma3d.org`.
...
`STOMA3D_PLATFORM_API_URL` must point to `services/platform-api`; the supplied
...
uses `STOMA3D_ALLOW_CI_DUMMY_WEB_ENV=true` with explicit dummy values so it can
...
`stoma3d-calibration-v1` card: ArUco `DICT_4X4_50`, marker ID 17, an exact 20 mm
...
The platform reads only `STOMA3D_PLATFORM_OIDC_ROLE_CLAIM` from the signed API
