...
def _locked_python_bom() -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="stoma3d-sbom-") as directory:
...
def _bom_component(package: Package) -> dict[str, Any]:
    ...
    component: dict[str, Any] = {
        ...
        "properties": [
            {"name": "stoma3d:ecosystem", "value": package.ecosystem},
            {
                "name": "stoma3d:present-in-release-environment",
...
def _notice_markdown(
    ...
    lines = [
        ...
        "This inventory is not legal advice. Stoma3D source is provided under the",
        "repository MIT license. Deployment-only research model files keep their own",
        "dataset and model terms and are not included in the public source package.",
        "",
        "## Inventory summary",
        "",
...
def _generate() -> tuple[str, str]:
    ...
    root_ref = "application:stoma3d-source"
    pnpm_lock_sha = _sha256_file(ROOT / "pnpm-lock.yaml")
    uv_lock_sha = _sha256_file(ROOT / "uv.lock")
    sbom = {
        "bomFormat": "CycloneDX",
        ...
        "metadata": {
            ...
            "properties": [
                {
                    "name": "stoma3d:pnpm-lock-sha256",
                    "value": pnpm_lock_sha,
                },
                {"name": "stoma3d:uv-lock-sha256", "value": uv_lock_sha},
            ],
        },
